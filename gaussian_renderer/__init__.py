...
def generate_neural_gaussians(viewpoint_camera, pc : GaussianModel, visible_mask=None, is_training=False,  ape_code=-1):
    ...
    # 수정: visible anchor feature만 runtime에 on-the-fly 복원
    # QAT mode: per-LOD fake-quantized float feat with STE; else: dequantize from stored uint8
    if pc.qat_mode:
        feat = pc.get_qat_anchor_feat(visible_mask)
    else:
        feat = pc.dequantize_visible_anchor_feat(visible_mask)
    level = pc.get_level[visible_mask]
    grid_offsets = pc.dequantize_visible_offset(visible_mask)
