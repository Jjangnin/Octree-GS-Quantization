...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0], ply_path=None, logger=None):
        ...
        if self.loaded_iter:
            ...
            quantized_path = os.path.join(point_cloud_path, "point_cloud_quantized.npz")
            ply_path_load = os.path.join(point_cloud_path, "point_cloud.ply")
            if os.path.exists(quantized_path):
                self.gaussians.load_gaussian(quantized_path)
            else:
                print("NPZ not found, falling back to PLY:", ply_path_load)
                self.gaussians.load_ply_sparse_gaussian(ply_path_load)
    ...
    def save(self, iteration, lod_bits_dict=None):
        point_cloud_path = os.path.join(self.model_path, "point_cloud/iteration_{}".format(iteration))
        self.gaussians.save_ply(os.path.join(point_cloud_path, "point_cloud.ply"))
        # 기하 정보(offset/opacity/scaling/rotation) + anchor feature 함께 양자화 저장
        # lod_bits_dict=None → per-LOD 8-bit default via get_lod_bits fallback (preserves legacy behavior)
        self.gaussians.save_gaussian(
            os.path.join(point_cloud_path, "point_cloud_quantized.npz"),
            lod_bits_dict=lod_bits_dict,
        )
